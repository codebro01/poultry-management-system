...
export const OrderCard = ({orders}) => {
  ...
  return (
    <>
          {/* Header */}
          <Box sx={{ textAlign: "center", display: "flex", justifyContent: "center", alignItems: "center", height: "70px" }} bgcolor={theme.palette.primary.main}>
          ...
          <Box sx={{ maxHeight: "380px", overflowY: "auto", padding: "16px" }}>
            <List>
              {orders.map((order) => (
                <Card key={order.id} sx={{ marginBottom: "12px", borderRadius: "12px", boxShadow: 0}}>
          ...
          {/* "See All" Link */}
          <Typography fontSize={12} color={theme.palette.text.darkGrey} position={'absolute'} bottom={"10px"} right={"10px"} sx={{ textDecoration: 'underline', cursor: "pointer" }}>
            See All
          </Typography>
    </>
  )
